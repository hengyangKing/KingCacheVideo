...
@interface KingLoaderURLConnection()<KingVideoRequestTaskDelegate>

@property (nonatomic, strong) NSMutableArray *pendingRequests;
@property (nonatomic, copy  ) NSString       *videoPath;

@end
@implementation KingLoaderURLConnection
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)dealWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if (!self.task) {
        self.task = [[KingVideoRequestTask alloc] init];
...
#pragma mark - KingVideoRequestTaskDelegate
